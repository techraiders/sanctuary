...
test('mapMaybe', function() {
  ...
  eq(S.mapMaybe.toString(), 'mapMaybe :: (Filterable f, Functor f) => (a -> Maybe b) -> f a -> f b');
  ...
  eq(S.mapMaybe(S.head)(Nil), Nil);
  eq(S.mapMaybe(S.head)(Cons([], Cons([], Cons([], Nil)))), Nil);
  eq(S.mapMaybe(S.head)(Cons([1, 2], Cons([3, 4], Cons([5, 6], Nil)))), Cons(1, Cons(3, Cons(5, Nil))));
  eq(S.mapMaybe(S.head)(Cons([1], Cons([], Cons([3], Cons([], Cons([5], Cons([], Nil))))))), Cons(1, Cons(3, Cons(5, Nil))));
